...
// Context 안에는 Provider 와 Consumer 라는 것이 존재한다.
// 이 둘은, Context를 이용하기 위해 필요한 컴포넌트들이다.
// Consumer 는 나중에 내보내줄 때 편하도록 SampleConsumer라고 부르도록 설정.
const { Provider, Consumer: SampleConsumer } = Context;

// Provider 에서 state 를 사용하기 위해서 컴포넌트를 새로 만들어줌.
class SampleProvider extends Component {
  ...
  // 나중에 변화를 일으키는 함수들을 전달해줄 때 함수 하나하나 일일히 전달하는 것이 아니라,
  // 객체 하나로 한꺼번에 전달하기 위해 만든 객체.
  actions = {
    setValue: value => {
      this.setState({ value });
    }
